...
namespace TinyCms.Services.Customers
{
    ...
    public class CustomerService : ICustomerService
    {
        ...
        /// <param name="sct">
        ///     Value indicating what shopping cart type to filter; userd when 'loadOnlyWithShoppingCart' param is
        ///     'true'
        /// </param>
        ...
        public virtual void DeleteCustomer(Customer customer)
        {
            ...
            if (customer.IsSystemAccount)
                throw new NopException(string.Format("System customer account ({0}) could not be deleted",
                    customer.SystemName));
        ...
        public virtual int DeleteGuestCustomers(DateTime? createdFromUtc, DateTime? createdToUtc)
        {
            if (_commonSettings.UseStoredProceduresIfSupported && _dataProvider.StoredProceduredSupported)
            {
                ...
                var totalRecordsDeleted = (pTotalRecordsDeleted.Value != DBNull.Value)
                    ? Convert.ToInt32(pTotalRecordsDeleted.Value)
                    : 0;
